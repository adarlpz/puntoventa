...
function registra_usr() {
    if ($("#nombre").val() == "" || $("#clave").val() == "" || $("#pass").val() == "") {
        window.location = "http://localhost/electron-products-test-master/src/views/error-user-register.php";
        $("#nombre").focus();
    } else {
        $.ajax({
            beforeSend: function() {},
            ...
            success: function(x) {
                if (x != '0') {
                    window.location = "http://localhost/electron-products-test-master/src/views/user-register.php";
                }
